...
// 用户赛事信息
	...
	//获取道具列表
	function getPropListAll()
	{
		$list = $this->mysql->getData("SELECT * FROM `lord_game_prop` ");
		if ( !$list )
		{
			return false;
		}
	...
	//购买道具
	function buyProp( $user, $propId, $is_free=0 )
	{
		$propId = intval($propId);
		if ( !$user || !is_array($user) || $propId <= 0 )
		{
			return 1;//购买失败
		}
		$propList = $this->getPropListAll();
		if ( !$propList )
		{
			return 1;//购买失败
		}
		...
		$prop = $propList[$propId];
		$gold = $prop['gold']+0;
		$coins = $prop['coins']+0;
		$coupon = $prop['coupon']+0;
		if ( !$is_free ) 
		{
			if ( !$gold && !$coins && !$coupon ) return 3;//不可购买
			if ( isset($user['propDress'][$propId]) ) return 4;//不需再买
			if ( $gold > $user['gold'] || $coins > $user['coins'] || $coupon > $user['coupon'] ) return 5;//金额不足
			if ( $prop['state'] ) return 6;//已经下架
			if ( !$prop['store'] ) return 7;//已经无货
		}
		$uid = $user['uid'];
		$dress = $user['propDress'];
		$dress[$propId] = intval(!in_array(1,$dress) || !count($dress) || (count($dress)==1 && isset($dress['1'])));
		$addU['gold'] = (!$is_free && $gold) ? ($gold * -1) : 0;
		$addU['coins'] = (!$is_free && $coins) ? ($coins * -1) : 0;
		$addU['coupon'] = (!$is_free && $coupon) ? ($coupon * -1) : 0;
		$newU['gold_'] = abs($addU['gold']);
		$newU['coins_'] = abs($addU['coins']);
		$newU['coupon_'] = abs($addU['coupon']);
		$newU['gold'] = $user['gold'] + $addU['gold'];
		$newU['coins'] = $user['coins'] + $addU['coins'];
		$newU['coupon'] = $user['coupon'] + $addU['coupon'];
		$newU['propDress'] = $setU['propDress'] = $dress;
		if ( $user['fd'] ) {
			$addU && $res = $this->incUserInfo($uid, $addU, 5);//5购买道具
			$setU && $res = setUser($uid, $setU);
		} else {
			$res = bobSql("UPDATE `lord_game_user` set `gold` = `gold` - $gold, `coins` = `coins` - $coins, `coupon` = `coupon` - $coupon WHERE `uid` = $uid");
		}
		$res = bobSql("INSERT INTO `lord_game_userprop` ( `uid`, `fromUid`, `propId`, `categoryId`, `propStart`, `propEnd`, `propState` ) VALUES ( $uid, 0, $propId, ".$prop['categoryId'].", ".time().", ".($prop['valid']?(time()+$prop['valid']*86400):0).", ".$dress[$propId]." )");
		return $newU;
	}
	...
	//$len		榜单长度
	//$uid		获取某用户的在前100名内的名次
	private function zList( $dateid, $flag, $is_week=0, $len=20, $uid=0 )
	{
		...
		if ($list) {
			...
			if ( $uid ) {
				$list_[0]['myval'] = isset($list[$uid]['val']) ? $list[$uid]['val'] : 0;
				$list_[0]['myrank'] = isset($list[$uid]['rank']) ? $list[$uid]['rank'] : 0;
			}
			return $list_[0];
		}
		//加事务锁	互斥锁
		...
		$list = array();
		$i=0;
		foreach ( $res as $uid_ => $val )
		{
			$i++;
			$nick = $this->getUserNick($uid_);
			$list[$uid_]= array('id'=>$uid_,'rank'=>$i, 'nick'=>$nick, 'val'=>$val);
		}
	...
	//获取榜单 普通场每日赢场排行榜
	function zListNormalDayWin( $dateid, $len=20, $uid=0 )
	{
		$flag = 'lord_list_normal_day_win_';
		return $this->zList($dateid, $flag, 0, $len, $uid);
	}

	//获取榜单 普通场每日赢钱排行榜
	function zListNormalDayEarn( $dateid, $len=20, $uid=0 )
	{
	...
	//获取榜单 普通场每日倍率排行榜
	function zListNormalDayMaxrate( $dateid, $len=20, $uid=0 )
	{
	...
	//获取榜单 普通场每周赢场排行榜
	function zListNormalWeekWin( $dateid, $len=20, $uid=0 )
	{
		$flag = 'lord_list_normal_week_win_';
		return $this->zList($dateid, $flag, 1, $len, $uid);
	}

	//获取榜单 普通场每周赢钱排行榜
	function zListNormalWeekEarn( $dateid, $len=20, $uid=0 )
	{
	...
	//获取榜单 竞技场每周积分排行榜
	function zListMatchWeekPoint( $dateid, $len=20, $uid=0 )
	{
	...
	//获取榜单 用户每日使用金币排行榜
	function zListUserDayCost( $dateid, $len=10, $uid=0 )
	{
	...
	//获取榜单 用户每日充值金币排行榜
	function zListUserDayRecharge( $dateid, $len=10, $uid=0 )
	{
	...
	//获取榜单 用户每月充值金币排行榜
	function zListUserMonthRecharge( $dateid, $len=10, $uid=0 )
	{
	...
	//更新入榜 普通场最大倍率榜单 日
	function zNormalDayMaxrate( $uid, $val )
	{
		$dateid = intval(date("Ymd"));
		$flag = 'lord_list_normal_day_maxrate_';
		$res = $this->zSet($dateid, $flag, array($uid=>$val));
		if (!$res) return false;
		return true;
	}

	//累加入榜 竞技场积分数量榜单 周、日
	function zMatchPoint( $uid, $val )
	{
		$dateid = intval(date("Ymd",time()-(date("N")-1)*86400));
		$flag = 'lord_list_match_week_point_';
		$res = $this->zAdd($dateid, $flag, array($uid=>$val));
	...
	function listGetTips( $ver, $channel = '' )
	{
		...
		foreach ( $list as $k => $v )
		{
			...
			// 	//加事务锁
			// 	$lockId = $this->key_list_tips.'_'.$v['id'];
			// 	$res = setLock($lockId, 1);
			// 	if ( $res ) {
	...
	//列表-获取-需要更新的素材列表
	function listGetFile( $ver )
	{
		$version = $this->getVersion('verfile');
		if ( $ver >= $version ) {
			return array();
		}
		...
		$list_ = $list;
		$list = array('verfile'=>$version,'insert'=>array(),'update'=>array(),'delete'=>array());
		foreach ( $list_ as $k => $v )
		{
			$vv = array();
	...
	//列表-设置-素材
	function listSetFile( $data )
	{
		if ( !isset($data['id']) || !$data['id'] || !isset($data['fileid']) || !$data['fileid'] || !isset($data['path']) || $data['path'] ) return false;
		return $this->redis->hset($this->key_list_file, $data['id'], $data);
	}


	//列表-获取-收件箱
	function listGetInbox( $uid )
	{
		$uid = intval($uid);
		if ( $uid <= 0 ) return array();
		$GAME = include(ROOT.'/conf/games.php');
		$sql = "SELECT * FROM `lord_user_inbox` WHERE `uid` = $uid AND `is_del` = 0 ORDER BY `id` DESC";
		$list = $this->mysql->getData($sql);
		$list = $list ? $list : array();
		$i = 0;
		$list2 = array();
		foreach ( $list as $k => $v )
		{
			if ( ( $v['type'] && $v['create_time'] < time() - $GAME['time_clear_inbox_type1'] ) || $v['create_time'] < time() - $GAME['time_clear_inbox_type0'] ) {
				unset($list[$k]);
				continue;
			}
			$list2[$k]['id'] = $v['id']+0;
		...
		$res = $this->redis->del($this->key_list_newmail_.$uid);
		return array_values($list2);
	}
	//列表-获取-新邮件
	function listGetNewMail( $uid )
	{
		$uid = intval($uid);
		if ( $uid <= 0 ) return array();
		$list = $this->redis->hgetall($this->key_list_newmail_.$uid);
		if ( $list ) {
			$res = $this->redis->del($this->key_list_newmail_.$uid);
	...
	function listSetNewMail( $data )
	{
		...
		$res = $this->incUserInfo($uid, array('mail_unread'=>1));
		return $this->redis->hmset($this->key_list_newmail_.$uid, array($id=>$mail));
	}
	//列表-获取-收件箱新邮件数量
	function getMailNewNum( $uid )
	{
		$uid = intval($uid);
		if ( $uid <= 0 ) return 0;
		$list = $this->listGetInbox($uid);
		$i = 0;
		foreach ( $list as $k => $v )
		{
			if ( !$v['is_read'] ) {
	...
	//列表-设置-收件箱邮件标记已读
	function listSetInboxRead( $uid, $id )
	{
		$uid = intval($uid);
		$id = intval($id);
		if ( $uid <= 0 || $id <= 0 ) return false;
		$sql = "UPDATE `lord_user_inbox` SET `is_read` = 1, `update_time` = ".time()." WHERE `id` = $id";
		bobSql($sql);//会有定时脚本把已读的邮件放到lord_user_unbox表
		$info = array('mail_unread'=>-1);
		$info = $this->incUserInfo($uid, $info);
		if ( $info && $info['mail_unread'] < 0 ) {
			$info['mail_unread'] = 0;
			setUser($uid, $info);
		}
		return $info['mail_unread'];
	}
	//列表-设置-收件箱邮件领取附件
	function listSetInboxGetItems( $user, $id )
	{
		$id = intval($id);
		if ( !is_array($user) || !isset($user['uid']) || $user['uid'] <= 0 || $id <= 0 ) return 1;
		...
		if ( !$res ) return 1;
		$res = $this->addUserPrize($user['uid'], $items, $user, 11);//领取邮件
		return $items;
	}
	//列表-设置-收件箱邮件删除
	...
	//列表-获取-活动
	function listGetTopic( $channel = '', $is_lobby=0 )
	{
		$list = array();
		$list_ = $this->redis->hgetall($this->key_list_topic);
		$list_ = $list_ ? $list_ : array();
		if ( !$list_ ) {
	...
	function listGetRoom()
	{
		...
		$list = $list ? $list : array();
		if ( !$list ) {
			$sql = "SELECT * FROM `lord_game_room` WHERE `is_del` = 0 ORDER BY `sort` DESC, `id` DESC";
			$data = $this->mysql->getData($sql);
			$data = $data ? $data : array();
			foreach ( $data as $k => $v )
			{
				$list[$v['id']]=array(
					...
					'name'=>$v['name'],
					'showRules'=>$v['showRules'] ? json_decode($v['showRules'],1) : array(),
					'baseCoins'	 => 0,
					'rateMax'	 => 0,
					'enter'		 => '',
					...
					'bytes'=>intval($v['bytes']),
					'desc'=>trim($v['desc']),
					'md5'=>$v['md5'],
					'package'=>$v['package'],
				);
			}
			$res = $this->redis->hmset($this->key_list_room, $list);
		}
		return array_values($list);
	}
	//列表-获取-消费记录
	function listGetCost( $uid )
	{
		$uid = intval($uid);
		if ( $uid <= 0 ) return array();
		$props = $this->getPropListAll();
		$sql = "SELECT `id`,`gold`,`coins`,`coupon`,`propId`,`date` FROM `lord_user_cost` WHERE `uid` = $uid ORDER BY `time` DESC";// AND `is_del` = 0
		$res = $this->mysql->getData($sql);
		$res = $res ? $res : array();
		return $res;
	}
	//数组-获取-用户救济数据
	function getdataUserTrial( $uid )
	{
		$uid = intval($uid);
		if ( $uid <= 0 ) return array('trial_count'=>99, 'trial_cooldown'=>86400);
		$data = $this->redis->hget('lord_trial_data', $uid);
		if ( !$data ) {
			$count = 0;
			$cooldown = microtime(1);
		} else {
			$data = explode("_", $data);
			$count = intval($data[0]);
			$cooldown = $data[1] + 0;
		}
		$cooldown = $cooldown - microtime(1);
		$cooldown = $cooldown > 0 ? ceil($cooldown) : 0;
		return array('trial_count'=>$count, 'trial_cooldown'=>$cooldown);
	}
	//数组-设置-用户救济数据
	function setdataUserTrial( $uid, $channel, $count )
	{
		$uid = intval($uid);
		if ( $uid <= 0 ) return false;
		$cooldown = $this->getvalueTrialCooldown($channel, $count);
		return $this->redis->hset('lord_trial_data', $uid, $count.'_'.($cooldown+microtime(1)));
	}
	//列表-获取-救济基数倍率
	function getlistTrialCoins( $channel, $count=0 )
	{
		...
		$res = $this->redis->hgetall('lord_trial_coins');
		if ( !($res && is_array($res)) ) {
			$res = array(); 
			$sql = "SELECT * FROM `lord_trial_coins` WHERE `is_del` = 0";
			...
			if ( $res_ ) {
				...
				$this->redis->hmset('lord_trial_coins', $res);
			}
		}
		$list = array();
		foreach ( $res as $k => $v )
		{
			if ( ($v['channel'] && $v['channel'] != $channel) || !$v['value'] || !$v['multiple'] || !$v['probability'] ) {
				continue;
			}
	...
	function getvalueTrialCooldown( $channel, $count=0 )
	{
		...
		$res = $this->redis->hgetall('lord_trial_cooldown');
		if ( !($res && is_array($res)) ) {
			$res = array();
			$sql = "SELECT * FROM `lord_trial_cooldown` WHERE `is_del` = 0";
			$res_= $this->mysql->getData($sql);
			if ( $res_ ) {
				foreach ( $res_ as $k => $v )
				{
					$res[$v['count']] = array('id'=>intval($v['id']),'channel'=>$v['channel'],'count'=>$v['count'],'cooldown'=>intval($v['cooldown']));
				}
				$this->redis->hmset('lord_trial_cooldown', $res);
			}
		}
		$list = array();
		foreach ( $res as $k => $v )
		{
			if ( !$v['count'] || !$v['cooldown'] ) {
				unset($res[$k]);
				continue;
			}
			if ( !$v['channel'] ) {
				$list[$v['count']] = $v['cooldown'];
			}
		}
		foreach ( $res as $k => $v )
		{
		...
		{
			if ( $count >= $v && ( isset($counts[$k+1]) && $count < $counts[$k+1] ) ) {
				$cooldown = $list[$v];
				break;
			}
		}
