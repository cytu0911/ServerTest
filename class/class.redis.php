...
class RD
{
	...
	function fixLock( $pattern, $fixPool )
	{
		...
		$old = ( $old && is_array($old) ) ? $old : array();
		//查找当前锁
		$now = array();
		$now_ = $this->keys($pattern);
		$now_ = ( $now_ && is_array($now_) ) ? $now_ : array();
		if ( $now_ ) 
		{
			foreach ( $now_ as $k => $v )
			{
		...
		{	//有当前锁
			if ( $old ) 
			{	//有上次锁，匹配上次锁和当前锁之间的重复锁
				$die = array_values(array_intersect($old, $now));
				if ( $die && is_array($die) ) 
				{	//找到重复锁，删除重复锁，筛选出正常的本次锁
	...
	function hset($key, $field, $value = null)
	{
		$key = trim(strval($key));
		if ($key === '') return false;
	...
	function hmset($key, $dataArray)
	{
		$key = trim(strval($key));
		if ( $key === '' ) return false;
		$dataArray = (is_array($dataArray) && $dataArray) ? $dataArray : array();
	...
	function hmget($key, $fieldArray)
	{
		$key = trim(strval($key));
		if ( $key === '' ) return false;
		$fieldArray = (is_array($fieldArray) && $fieldArray) ? $fieldArray : array();
		...
		$res = (is_array($res) && $res) ? $res : array();
		foreach($res as $k => $v){
	...
	function zinter( $newkey, $keys, $com=array(), $agg="SUM" )
	{
		$new = trim(strval($newkey));
		$len = count($keys);
		if ( empty($new) || $len < 2 )  return false;
		$com = ( $com && is_array($com) && count($com) == $len ) ? $com : array_pad(array(1), $len, 1);
		foreach ( $com as $k => $v )
		{
			$com[$k] = $v + 0;
		}
		$com = array_values($com);
		$agg = in_array($agg,array("SUM","MIN","MAX")) ? $agg : "SUM";
		try {
	...
	function zunion( $newkey, $keys, $com=array(), $agg="SUM" )
	{
		$new = trim(strval($newkey));
		$len = count($keys);
		if ( empty($new) || $len < 2 )  return false;
	...
	function zrangebyscore( $key, $min="-inf", $max="+inf", $options=array() )
	{
		$key = trim(strval($key));
		$min = $min == "-inf" ? $min : ( $min + 0 );
		$max = $max == "+inf" ? $max : ( $max + 0 );
		if ( empty($key) )  return false;
		$opt = array('withscores'=>true,'limit'=>array(0,10));
		if ($options && is_array($options)) {
			$options['withscores'] = isset($options['withscores']) ? !!($options['withscores']) : true;
			$options['limit'] = ( isset($options['limit']) && is_array($options['limit']) && count($options['limit']) == 2 ) ? $options['limit'] : array(0,10);
		}
		else{
			$options = $opt;
		}
	...
	function zrevrangebyscore( $key, $min="-inf", $max="+inf", $options=array() )
	{
		$key = trim(strval($key));
		$min = $min == "-inf" ? $min : ( $min + 0 );
		$max = $max == "+inf" ? $max : ( $max + 0 );
